...
## Python examples
Files under `examples/job_launching/python/` create ClearML tasks and enqueue them to queues:
...
## Bash/PBS examples
Files under `examples/job_launching/bash/` mirror the Python flow but reference bash payload scripts:
...
**Files:**
- `examples/job_launching/pattern_b/submit_pattern_b.py` (enqueue the single allocation)
- `examples/job_launching/pattern_b/run_local_tasks.py` (runs inside the allocation)
- `examples/job_launching/pattern_b/subtask_worker.py` (example subtask)

**Run:**
```bash
python examples/job_launching/pattern_b/submit_pattern_b.py --queue crux --num-nodes 6
```
