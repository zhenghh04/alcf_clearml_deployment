...
```bash
cp server/globus_connector/.env.example server/globus_connector/.env
```
...
```bash
pip install -r server/globus_connector/requirements.txt
set -a; source server/globus_connector/.env; set +a
