...
CONNECTOR_DIR="${ROOT_DIR}/server/globus_connector"
...
pkill -f "uvicorn server\\.globus_connector\\.main:app" >/dev/null 2>&1 || true
...
if [[ "${DETACH}" == "1" ]]; then
  nohup uvicorn server.globus_connector.main:app --host "${HOST}" --port "${PORT}" > "${LOG_FILE}" 2>&1 &
  echo "Globus connector started in background on ${HOST}:${PORT} (pid=$!)."
  echo "Logs: ${LOG_FILE}"
else
  exec uvicorn server.globus_connector.main:app --host "${HOST}" --port "${PORT}"
fi
